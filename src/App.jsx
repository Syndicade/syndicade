...
import ProgramDiscovery from './pages/ProgramDiscovery';
...
function App() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center" style={{ background: '#F8FAFC' }} role="status" aria-label="Loading Syndicade">
        <div className="text-center">
          <img src="/mascot-loading.png" alt="" aria-hidden="true" style={{ width: '180px', display: 'block', margin: '0 auto 16px', mixBlendMode: 'multiply' }} />
          <p style={{ color: '#64748B', fontSize: '14px', fontWeight: 600, letterSpacing: '0.5px' }}>Loading Syndicade...</p>
        </div>
      </div>
    );
  }
...
function AppShell({ session }) {
  ...
  // Pages that manage their own Header/Footer
  var hideChrome = (
    path === '/' ||
    path === '/home' ||
    path === '/about' ||
    ...
    <div className="min-h-screen bg-gray-50" style={{ display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1 }}>
        <Routes>
          ...
          {/* ── Organization pages ─────────────────────────────── */}
          <Route path="/organizations/:organizationId" element={session ? <OrgLayout /> : <Navigate to="/login" replace />}>
