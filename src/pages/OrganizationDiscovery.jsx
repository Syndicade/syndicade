...
function OrgCardSkeleton() {
  return (
    <div
      style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '20px 16px 14px', display: 'flex', flexDirection: 'column', gap: '12px', boxShadow: '3px 4px 14px rgba(0,0,0,0.08), 0 1px 3px rgba(0,0,0,0.05)' }}
      aria-hidden="true"
    >
...
export default function OrganizationDiscovery() {
  ...
  var fetchOrgs = useCallback(function () {
    ...
    var run = async function () {
      try {
        ...
        mascotErrorToast('Failed to load organizations', 'Check your connection and try again.');
      } finally {
        setLoading(false);
      }
    };
  ...
  function handleRequestLocation() {
    if (!navigator.geolocation) { setLocationError(true); mascotErrorToast('Location unavailable', 'Your browser does not support geolocation.'); return; }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      function () {
        ...
        setLocationLoading(false);
        mascotErrorToast('Could not get location', 'Please enable location access and try again.');
      }
    );
  }
  ...
  (filters.causeAreas || []).forEach(function (c) { activeChips.push({ type: 'causeArea', value: c, label: c }); });
  ...
  if (filters.state && filters.state.length >= 2) activeChips.push({ type: 'state', value: filters.state, label: 'State: ' + filters.state });
  if (filters.city && filters.city.length >= 2)   activeChips.push({ type: 'city',  value: filters.city,  label: 'City: ' + filters.city });
  if (filters.zip && filters.zip.length >= 3)     activeChips.push({ type: 'zip',   value: filters.zip,   label: 'ZIP: ' + filters.zip });
  ...
  function removeChip(chip) {
    ...
    else if (chip.type === 'causeArea') setFilters(function (p) { return Object.assign({}, p, { causeAreas: (p.causeAreas || []).filter(function (c) { return c !== chip.value; }) }); });
    ...
    else if (chip.type === 'zip')   setFilters(function (p) { return Object.assign({}, p, { zip: '' }); });
  }
    ...
    <>
      ...
      <main id="main-content">
        ...
        >
          ...
          <div style={{ background: '#FFFFFF', borderBottom: '1px solid #E2E8F0', position: 'sticky', top: 64, zIndex: 30 }}>
            <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '12px 20px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>

              <div style={{ display: 'inline-flex', background: '#F1F5F9', border: '1px solid #E2E8F0', borderRadius: '10px', padding: '3px', flexShrink: 0 }} role="group" aria-label="Organization type filter">
                <button
                ...
                >
                  <BadgeCheck size={13} aria-hidden="true" />Nonprofits
                </button>
                <button
              ...
              <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
                ...
                />
                {keyword && (
                  <button onClick={function () { setKeyword(''); }} aria-label="Clear search" style={{ position: 'absolute', right: '10px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', display: 'flex', alignItems: 'center' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500 rounded">
              ...
              <button
                ...
                aria-label="Open filters" aria-expanded={mobileFiltersOpen}
                ...
                <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Filters">
                  ...
                  <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '300px', maxWidth: '100%', background: '#FFFFFF', overflowY: 'auto', padding: '16px', boxShadow: '4px 0 24px rgba(0,0,0,0.15)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                      ...
                      <button onClick={function () { setMobileFiltersOpen(false); }} aria-label="Close filters" style={{ padding: '6px', borderRadius: '8px', background: 'none', border: 'none', cursor: 'pointer', color: '#64748B', display: 'flex' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              <main style={{ flex: 1, minWidth: 0 }} aria-label="Organization results">
                ...
                <div style={{ marginBottom: '20px' }}>
                  <h1 style={{ fontSize: '22px', fontWeight: 800, color: '#0E1523', marginBottom: '4px' }}>
                    {verifiedOnly ? 'Verified Nonprofits' : 'Discover Local Organizations'}
                  </h1>
                  <p style={{ fontSize: '13px', color: '#64748B' }}>
                    {verifiedOnly ? 'Verified 501(c)(3) nonprofits in your community.' : 'Explore nonprofits and community groups in your area.'}
                  </p>
                </div>
                ...
                {activeChips.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginBottom: '16px' }} role="list" aria-label="Active filters">
                    {activeChips.map(function (chip) {
                      return (
                        <span key={chip.type + '-' + chip.value} role="listitem" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#F1F5F9', border: '1px solid #E2E8F0', color: '#374151', borderRadius: '99px', padding: '3px 10px', fontSize: '11px', fontWeight: 600 }}>
                          {chip.type === 'tag' && <Tag size={9} color="#F5B731" aria-hidden="true" />}
                          {chip.label}
                          <button onClick={function () { removeChip(chip); }} aria-label={'Remove filter: ' + chip.label} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748B', display: 'flex', alignItems: 'center', padding: '0' }} className="focus:outline-none focus:ring-1 focus:ring-blue-400 rounded-full">
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', flexWrap: 'wrap', gap: '8px' }}>
                  <p style={{ fontSize: '13px', color: '#64748B' }} aria-live="polite" aria-atomic="true">
                    {!loading && !error && (displayOrgs.length + ' results')}
                  </p>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <label htmlFor="sort-select" style={{ fontSize: '12px', color: '#64748B', whiteSpace: 'nowrap' }}>Sort by:</label>
                    <div style={{ position: 'relative' }}>
                      <select id="sort-select" value={sortBy} onChange={function (e) { setSortBy(e.target.value); }} style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '8px', padding: '6px 28px 6px 10px', fontSize: '12px', color: '#374151', appearance: 'none', cursor: 'pointer' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500">
                ...
                {/* Skeletons */}
                {loading && (
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }} aria-label="Loading organizations" aria-busy="true">
                ...
                {!loading && error && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', textAlign: 'center' }} role="alert">
                    ...
                    <h2 style={{ fontSize: '17px', fontWeight: 700, color: '#0E1523', marginBottom: '8px' }}>Something went wrong</h2>
                    <p style={{ fontSize: '13px', color: '#64748B', marginBottom: '24px', maxWidth: '360px' }}>Failed to load organizations. Please try again.</p>
                    <button onClick={fetchOrgs} style={{ padding: '10px 24px', background: '#3B82F6', color: '#FFFFFF', fontSize: '13px', fontWeight: 600, borderRadius: '10px', border: 'none', cursor: 'pointer' }} className="hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">Try Again</button>
                  </div>
                )}

                {/* Empty */}
                {!loading && !error && displayOrgs.length === 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', textAlign: 'center' }}>
                    ...
                    <h2 style={{ fontSize: '17px', fontWeight: 700, color: '#0E1523', marginBottom: '8px' }}>
                      {verifiedOnly ? 'No verified nonprofits found' : 'No organizations found'}
                    </h2>
                    <p style={{ fontSize: '13px', color: '#64748B', marginBottom: '24px', maxWidth: '360px' }}>
                      {verifiedOnly ? 'Try adjusting your filters, or switch to All Organizations to see all groups.' : 'Try adjusting your filters or search terms.'}
                    </p>
                    <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', justifyContent: 'center' }}>
                      ...
                      <button onClick={handleReset} style={{ padding: '10px 24px', background: '#3B82F6', color: '#FFFFFF', fontSize: '13px', fontWeight: 600, borderRadius: '10px', border: 'none', cursor: 'pointer' }} className="hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2">
                        Reset Filters
                      </button>
                    </div>
                  </div>
                  ...
                  <>
                    ...
                    {totalPages > 1 && (
                      <nav style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginTop: '32px' }} aria-label="Pagination">
                        <button onClick={function () { setPage(function (p) { return Math.max(1, p - 1); }); }} disabled={page === 1} aria-label="Previous page" style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '7px 14px', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '12px', color: page === 1 ? '#CBD5E1' : '#374151', cursor: page === 1 ? 'not-allowed' : 'pointer' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500">
                        ...
                        <div style={{ display: 'flex', gap: '4px' }}>
                          {Array.from({ length: Math.min(5, totalPages) }, function (_, i) {
                            ...
                            return (
                              <button key={pn} onClick={function () { setPage(pn); }} aria-label={'Page ' + pn} aria-current={pn === page ? 'page' : undefined} style={{ width: '34px', height: '34px', borderRadius: '8px', fontSize: '12px', fontWeight: 600, border: '1px solid ' + (pn === page ? '#3B82F6' : '#E2E8F0'), background: pn === page ? '#3B82F6' : '#FFFFFF', color: pn === page ? '#FFFFFF' : '#374151', cursor: 'pointer' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500">
                                {pn}
                              </button>
                            );
                          })}
                        </div>
                        <button onClick={function () { setPage(function (p) { return Math.min(totalPages, p + 1); }); }} disabled={page === totalPages} aria-label="Next page" style={{ display: 'flex', alignItems: 'center', gap: '4px', padding: '7px 14px', background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '8px', fontSize: '12px', color: page === totalPages ? '#CBD5E1' : '#374151', cursor: page === totalPages ? 'not-allowed' : 'pointer' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500">
