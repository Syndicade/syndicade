...
function ProgramDetail() {
  ...
  async function init() {
    ...
    try {
      ...
      var [progRes, orgRes] = await Promise.all([
        ...
        supabase.from('organizations').select('id, name, logo_url').eq('id', organizationId).single(),
      ]);
  ...
  async function fetchMyReg(uid, pid) {
    var res = await supabase.from('program_registrations').select('id, status, created_at').eq('program_id', pid).eq('user_id', uid).maybeSingle();
  ...
  async function fetchRegistrations(pid) {
    setLoadingReg(true);
    var res = await supabase.from('program_registrations').select('id, status, created_at, notes, user_id').eq('program_id', pid).in('status', ['enrolled', 'pending']).order('created_at', { ascending: true });
  ...
  async function handleRegister() {
    ...
    if (program.capacity != null && countRes.count >= program.capacity) { toast.error('This program is full.'); setActing(false); return; }
    var status = program.requires_approval ? 'pending' : 'enrolled';
    var res = await supabase.from('program_registrations').insert({ program_id: program.id, user_id: currentUserId, organization_id: organizationId, status: status });
    setActing(false);
    if (res.error) {
      if (res.error.code === '23505') { toast.error('You are already registered.'); return; }
      mascotErrorToast('Registration failed.', 'Please try again.');
      return;
    }
    setMyReg({ status: status, created_at: new Date().toISOString() });
    mascotSuccessToast(status === 'enrolled' ? 'Registered!' : 'Request submitted!', status === 'enrolled' ? 'You are now enrolled in ' + program.name + '.' : 'Your registration is pending admin approval.');
    try {
      var notifModule = await import('../lib/notificationService');
      var mRes = await supabase.from('members').select('first_name, last_name').eq('user_id', currentUserId).single();
      var memberName = mRes.data ? mRes.data.first_name + ' ' + mRes.data.last_name : 'A member';
      await notifModule.notifyOrgAdmins({ organizationId, type: 'program_registration', title: program.name, message: memberName + (status === 'enrolled' ? ' registered for ' : ' requested to join ') + program.name + '.', link: '/organizations/' + organizationId + '/programs/' + program.id, excludeUserId: currentUserId });
  ...
  function handleCancelRegistration() {
    openConfirm({
      title: 'Cancel registration?', message: 'You will be removed from ' + program.name + '. You can re-register later if spots are available.',
      confirmLabel: 'Cancel Registration', confirmBg: '#EF4444', confirmRing: 'focus:ring-red-500', confirmHover: 'hover:bg-red-600',
      onConfirm: async function() {
        closeConfirm();
        var res = await supabase.from('program_registrations').update({ status: 'cancelled', updated_at: new Date().toISOString() }).eq('id', myReg.id);
  ...
  async function updateRegStatus(regId, memberName, newStatus) {
    var authRes = await supabase.auth.getUser();
    var res = await supabase.from('program_registrations').update({ status: newStatus, reviewed_by: authRes.data.user.id, reviewed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', regId);
  ...
  function handleRemoveMember(reg) {
    openConfirm({
      title: 'Remove ' + reg.member_name + '?', message: reg.member_name + ' will be removed from this program.',
      confirmLabel: 'Remove', confirmBg: '#EF4444', confirmRing: 'focus:ring-red-500', confirmHover: 'hover:bg-red-600',
      onConfirm: async function() {
        closeConfirm();
        var authRes = await supabase.auth.getUser();
        var res = await supabase.from('program_registrations').update({ status: 'cancelled', reviewed_by: authRes.data.user.id, reviewed_at: new Date().toISOString(), updated_at: new Date().toISOString() }).eq('id', reg.id);
  ...
  async function saveNote(regId) {
    var res = await supabase.from('program_registrations').update({ notes: noteText, updated_at: new Date().toISOString() }).eq('id', regId);
    ...
    return (
      <div style={{ minHeight: '100vh', background: PAGE_BG, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Inter','Segoe UI',system-ui,sans-serif" }}>
        <div style={{ textAlign: 'center', padding: '48px 24px', maxWidth: '360px' }}>
          ...
          <button onClick={function() { navigate('/organizations/' + organizationId + '/programs'); }} style={{ padding: '10px 20px', background: '#3B82F6', color: '#FFFFFF', fontSize: '13px', fontWeight: 700, borderRadius: '8px', border: 'none', cursor: 'pointer' }} className="hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500">View All Programs</button>
        </div>
      </div>
    );
  }
  ...
  function statusBadgeStyle(status) {
    var cfg = { active: { bg: 'rgba(34,197,94,0.12)', color: '#22C55E' }, upcoming: { bg: 'rgba(59,130,246,0.12)', color: '#3B82F6' }, closed: { bg: 'rgba(100,116,139,0.12)', color: '#64748B' } };
  ...
  var orgInitials = organization && organization.name ? organization.name.split(' ').slice(0,2).map(function(w) { return w[0]; }).join('').toUpperCase() : '?';
  ...
  return (
    <>
        ...
        {program.image_url ? (
          <div style={{ position: 'relative', width: '100%', height: '280px', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'absolute', bottom: '24px', left: '24px', right: '24px' }}>
              <button onClick={function() { navigate('/organizations/' + organizationId + '/programs'); }} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '12px', fontWeight: 600, color: 'rgba(255,255,255,0.85)', background: 'rgba(0,0,0,0.3)', border: 'none', cursor: 'pointer', padding: '5px 10px', borderRadius: '6px', marginBottom: '10px', backdropFilter: 'blur(4px)' }} className="hover:text-white focus:outline-none focus:ring-2 focus:ring-white rounded" aria-label="Back to Programs">
                <Icon path={ICONS.chevLeft} className="h-3.5 w-3.5" />Programs
              </button>
            </div>
          </div>
        ) : (
          <div style={{ padding: '32px 24px 0' }}>
            <button onClick={function() { navigate('/organizations/' + organizationId + '/programs'); }} style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '13px', fontWeight: 600, color: MUTED, background: 'none', border: 'none', cursor: 'pointer', padding: '4px 0', marginBottom: '24px' }} className="hover:text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded" aria-label="Back to Programs">
              <Icon path={ICONS.chevLeft} className="h-4 w-4" />Programs
            </button>
          </div>
        )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', minWidth: 0 }}>
              ...
              <div style={{ background: CARD_BG, border: '1px solid ' + BDR, borderRadius: '16px', padding: '28px 32px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', marginBottom: '14px' }}>
                  <h1 id="pd-title" style={{ fontSize: '26px', fontWeight: 800, color: TEXT, margin: 0, lineHeight: 1.25, flex: 1 }}>{program.name}</h1>
                  {!isAdmin && (
                    <button onClick={toggleSave} style={{ padding: '6px', borderRadius: '8px', background: 'none', border: '1px solid ' + BDR, cursor: 'pointer', color: isSaved ? '#F5B731' : MUTED, flexShrink: 0 }} className={'focus:outline-none focus:ring-2 focus:ring-yellow-400 ' + (isSaved ? '' : 'hover:border-yellow-400 hover:text-yellow-500')} aria-label={isSaved ? 'Remove bookmark' : 'Bookmark this program'} aria-pressed={isSaved}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  ...
                  {program.publish_to_discovery && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: 'rgba(139,92,246,0.12)', color: '#8B5CF6' }}><Icon path={ICONS.globe} className="h-3 w-3" />On Discover</span>}
                  {program.requires_approval && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: 'rgba(100,116,139,0.1)', color: '#475569' }}><Icon path={ICONS.lock} className="h-3 w-3" />Approval required</span>}
                  {!program.registration_open && !isClosed && <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: 'rgba(239,68,68,0.1)', color: '#EF4444' }}>Registration closed</span>}
                  {isAdmin && !program.is_public && <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: 'rgba(245,183,49,0.12)', color: '#B45309' }}>Hidden</span>}
                  {!showEnrolled && isAdmin && <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: 'rgba(100,116,139,0.1)', color: MUTED }}><Icon path={ICONS.eyeOff} className="h-3 w-3" />Enrollment hidden from public</span>}
                </div>
              </div>
                ...
                <div style={{ background: CARD_BG, border: '1px solid ' + BDR, borderRadius: '16px', padding: '20px 28px' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }} role="list" aria-label="Program tags">
                    {program.tags.map(function(tag) {
                      return <span key={tag} role="listitem" style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontWeight: 600, padding: '4px 10px', borderRadius: '6px', background: ELEVATED, color: TEXT2, border: '1px solid ' + BDR }}><Icon path={ICONS.tag} className="h-3 w-3" style={{ color: MUTED }} />{tag}</span>;
                    })}
                  </div>
                </div>
              )}
                ...
                <div style={{ background: CARD_BG, border: '1px solid ' + BDR, borderRadius: '16px', padding: '20px 28px' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div>
                      ...
                      {program.contact_email && <a href={'mailto:' + program.contact_email} style={{ fontSize: '13px', color: '#3B82F6', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '4px' }} className="hover:underline focus:outline-none focus:ring-2 focus:ring-blue-500 rounded" aria-label={'Email ' + (program.contact_name || program.contact_email)}><Icon path={ICONS.mail} className="h-3.5 w-3.5" />{program.contact_email}</a>}
                    </div>
                  </div>
                </div>
              )}
                ...
                <section aria-labelledby="pd-pending-title">
                  <div style={{ background: CARD_BG, border: '1px solid ' + BDR, borderRadius: '16px', padding: '24px 28px' }}>
                    <h2 id="pd-pending-title" style={{ fontSize: '15px', fontWeight: 800, color: TEXT, margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      Pending Approval
                      {pending.length > 0 && <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '20px', height: '20px', borderRadius: '50%', background: '#F59E0B', color: '#FFFFFF', fontSize: '11px', fontWeight: 800 }}>{pending.length}</span>}
                    </h2>
                    {loadingReg ? (
              ...
              <div style={{ background: CARD_BG, border: '1px solid ' + BDR, borderRadius: '16px', padding: '24px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                ...
                {(isClosed || !regOpen) && (
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: '8px', padding: '10px 12px', background: 'rgba(100,116,139,0.08)', border: '1px solid rgba(100,116,139,0.2)', borderRadius: '8px', marginBottom: '16px' }}>
                    ...
                    <p style={{ fontSize: '12px', color: MUTED, margin: 0, lineHeight: 1.5 }}>{isClosed ? 'This program has ended and is no longer accepting registrations.' : 'Registration is currently closed for this program.'}</p>
                  </div>
                )}

                {isAdmin && <p style={{ fontSize: '13px', color: MUTED, margin: 0, lineHeight: 1.5 }}>You're viewing as an admin. Registration actions are available to members.</p>}
                ...
                {!isAdmin && isEnrolled && (
                  <div>
                    ...
                    {myReg && myReg.created_at && <p style={{ fontSize: '12px', color: MUTED, margin: '0 0 12px' }}>Enrolled on {new Date(myReg.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</p>}
                ...
                {canRegister && (
                  <div>
                    {program.requires_approval && <p style={{ fontSize: '12px', color: MUTED, margin: '0 0 12px', lineHeight: 1.5 }}>This program requires admin approval. Submit a request and an admin will review it.</p>}
                    {isFull ? (
                      ...
                      <button onClick={handleRegister} disabled={acting} style={{ width: '100%', padding: '12px', background: '#3B82F6', color: '#FFFFFF', border: 'none', borderRadius: '10px', fontSize: '14px', fontWeight: 700, cursor: acting ? 'not-allowed' : 'pointer', opacity: acting ? 0.7 : 1 }} className="hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" aria-busy={acting}>
                        {acting ? 'Submitting...' : (program.requires_approval ? 'Request to Join' : 'Register Now')}
                      </button>
                    )}
                  </div>
                )}

                {!isAdmin && !myReg && !canRegister && !isFull && <p style={{ fontSize: '13px', color: MUTED, margin: 0 }}>Registration is not currently available.</p>}
              </div>
      ...
      <ConfirmModal isOpen={confirmModal.open} title={confirmModal.title} message={confirmModal.message} confirmLabel={confirmModal.confirmLabel} confirmBg={confirmModal.confirmBg} confirmRing={confirmModal.confirmRing} confirmHover={confirmModal.confirmHover} onConfirm={confirmModal.onConfirm || function() {}} onCancel={closeConfirm} />
    </>
  );
}
