...
function FundingCardSkeleton() {
  return (
    <div style={{ background: cardBg, border: '1px solid ' + borderColor, borderRadius: '12px', padding: '20px', marginBottom: '10px' }} aria-hidden="true" className="animate-pulse">
    ...
    <div>
      ...
      <FundFilterSection label="Location" defaultOpen={true}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          <input value={filters.city} onChange={function(e) { onChange('city', e.target.value); }} placeholder="City (e.g. Toledo)" style={{ width: '100%', padding: '7px 10px', border: '1px solid ' + borderColor, borderRadius: '7px', fontSize: '13px', color: textPrimary, background: cardBg, outline: 'none', boxSizing: 'border-box' }} aria-label="Filter by city" className="focus:ring-2 focus:ring-blue-500" />
          <input value={filters.state || ''} onChange={function(e) { onChange('state', e.target.value); }} placeholder="State (e.g. Ohio)" style={{ width: '100%', padding: '7px 10px', border: '1px solid ' + borderColor, borderRadius: '7px', fontSize: '13px', color: textPrimary, background: cardBg, outline: 'none', boxSizing: 'border-box' }} aria-label="Filter by state" className="focus:ring-2 focus:ring-blue-500" />
          <input value={filters.zip || ''} onChange={function(e) { onChange('zip', e.target.value); }} placeholder="ZIP Code" style={{ width: '100%', padding: '7px 10px', border: '1px solid ' + borderColor, borderRadius: '7px', fontSize: '13px', color: textPrimary, background: cardBg, outline: 'none', boxSizing: 'border-box' }} aria-label="Filter by ZIP code" className="focus:ring-2 focus:ring-blue-500" />
        </div>
      </FundFilterSection>
      ...
      <FundFilterSection label="Cause Area" defaultOpen={false}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
          {tagSets.causeAreas.map(function(tag) {
            ...
            return (
              <button key={tag} onClick={function() { onToggleTag('causeAreas', tag); }}
                style={{ padding: '3px 9px', borderRadius: '99px', fontSize: '11px', fontWeight: active ? 700 : 500, border: '1px solid ' + (active ? '#3B82F6' : borderColor), background: active ? '#EFF6FF' : 'transparent', color: active ? '#3B82F6' : textSecondary, cursor: 'pointer', transition: 'all 0.1s' }}
                className="focus:outline-none focus:ring-2 focus:ring-blue-400" aria-pressed={active}>
                {tag}
              </button>
            );
          })}
      ...
      <FundFilterSection label="Audience Served" defaultOpen={false}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {tagSets.audience.map(function(tag) {
            ...
            return (
              <label key={tag} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: checked ? '#3B82F6' : textSecondary, fontWeight: checked ? 700 : 400 }} className="hover:bg-slate-50">
                <input type="checkbox" checked={checked} onChange={function() { onToggleTag('audience', tag); }} style={{ accentColor: '#3B82F6', width: '13px', height: '13px', flexShrink: 0 }} aria-label={tag} />
                {tag}
              </label>
            );
          })}
      ...
      <FundFilterSection label="Languages" defaultOpen={false}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
          {tagSets.languages.map(function(tag) {
            ...
            return (
              <label key={tag} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '5px 8px', borderRadius: '6px', cursor: 'pointer', fontSize: '13px', color: checked ? '#3B82F6' : textSecondary, fontWeight: checked ? 700 : 400 }} className="hover:bg-slate-50">
                <input type="checkbox" checked={checked} onChange={function() { onToggleTag('languages', tag); }} style={{ accentColor: '#3B82F6', width: '13px', height: '13px', flexShrink: 0 }} aria-label={tag} />
                {tag}
              </label>
            );
          })}
...
function ApplyModal({ item, onClose, currentUser }) {
  ...
  useEffect(function() { if (firstFieldRef.current) firstFieldRef.current.focus(); }, []);
  useEffect(function() {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    var payload = { funding_id: item.id, organization_id: item.organization_id, applicant_name: form.name.trim(), applicant_email: form.email.trim(), message: form.message.trim() || null };
    ...
    setSaving(false);
    if (r.error) { mascotErrorToast('Application failed.', 'Something went wrong. Please try again.'); setErrors({ submit: 'Something went wrong. Please try again.' }); return; }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 50 }} role="dialog" aria-modal="true" aria-labelledby="fund-apply-title" onClick={function(e) { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: cardBg, borderRadius: '16px', width: '100%', maxWidth: '480px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid ' + borderColor }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted, padding: '4px' }} className="focus:outline-none focus:ring-2 focus:ring-blue-500 rounded" aria-label="Close"><X size={18} /></button>
        </div>
        <div style={{ padding: '24px' }}>
          {submitted ? (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ fontSize: '13px', color: textSecondary, lineHeight: 1.6, marginBottom: '20px' }}>The organization will be in touch at <strong>{form.email}</strong>.</p>
            ...
            <>
              <div style={{ marginBottom: '16px' }}>
                ...
                <input ref={firstFieldRef} id="fund-apply-name" value={form.name} onChange={function(e) { setField('name', e.target.value); }} placeholder="Jane Smith" style={Object.assign({}, inputStyle, errors.name ? { borderColor: '#EF4444' } : {})} aria-required="true" aria-describedby={errors.name ? 'fund-name-error' : undefined} className="focus:ring-2 focus:ring-blue-500" />
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                <input id="fund-apply-email" type="email" value={form.email} onChange={function(e) { setField('email', e.target.value); }} placeholder="jane@example.com" style={Object.assign({}, inputStyle, errors.email ? { borderColor: '#EF4444' } : {})} aria-required="true" aria-describedby={errors.email ? 'fund-email-error' : undefined} className="focus:ring-2 focus:ring-blue-500" />
              ...
              <button onClick={handleSubmit} disabled={saving} style={{ width: '100%', padding: '11px', background: '#16A34A', color: '#fff', border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: 700, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }} className="hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500" aria-disabled={saving}>
      ...
      <div style={{ width: '100%', maxWidth: '520px', background: cardBg, height: '100%', overflowY: 'auto', boxShadow: '-4px 0 32px rgba(0,0,0,0.12)' }}>
        <div style={{ padding: '24px 24px 20px', borderBottom: '1px solid ' + borderColor, position: 'sticky', top: 0, background: cardBg, zIndex: 1 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
            ...
            <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', color: textMuted, padding: '4px', flexShrink: 0 }} className="focus:outline-none focus:ring-2 focus:ring-blue-500 rounded" aria-label="Close"><X size={20} /></button>
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px', marginTop: '14px' }}>
            <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: typeColor.bg, color: typeColor.color }}>{typeLabel ? typeLabel.label : item.funding_type}</span>
            {amount && <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: '#F0FDF4', color: '#16A34A' }}>{amount}</span>}
            {item.deadline && !isExpired && <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: elevatedBg, color: textMuted }}>Due {new Date(item.deadline).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
            {isExpired && <span style={{ fontSize: '11px', fontWeight: 700, padding: '3px 10px', borderRadius: '99px', background: '#FEF2F2', color: '#DC2626' }}>Closed</span>}
          </div>
        </div>
      ...
      <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '10px' }}>
            <span style={{ fontSize: '11px', fontWeight: 700, padding: '2px 8px', borderRadius: '99px', background: typeColor.bg, color: typeColor.color }}>{typeLabel ? typeLabel.label : item.funding_type}</span>
            {amount && <span style={{ fontSize: '11px', fontWeight: 800, padding: '2px 8px', borderRadius: '99px', background: '#F0FDF4', color: '#16A34A' }}>{amount}</span>}
          </div>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '10px', alignItems: 'center', borderTop: '1px solid ' + borderColor, paddingTop: '10px' }}>
            ...
            {item.posting_url && <span style={{ fontSize: '12px', color: textMuted, display: 'flex', alignItems: 'center', gap: '4px' }}><Paperclip size={11} aria-hidden="true" />Document attached</span>}
...
export default function FundingDiscovery() {
  ...
  useEffect(function() {
    getDiscoveryFilterTags('funding').then(function(t) { setTagSets(t); });
  }, []);
  ...
  useEffect(function() { loadItems(); loadCurrentUser(); }, []);
  ...
  async function loadCurrentUser() {
    ...
    setCurrentUser({ id: userId, email: session.user.email, first_name: profileRes.data ? profileRes.data.first_name : '', last_name: profileRes.data ? profileRes.data.last_name : '' });
  }

  async function loadItems() {
    setLoading(true); setError(null);
    var r = await supabase.from('org_funding').select('*').eq('visibility', 'public').is('closed_at', null).order('created_at', { ascending: false });
    if (r.error) { setError(r.error.message); setLoading(false); return; }
    var data = (r.data || []).filter(function(item) { return !item.deadline || new Date(item.deadline) >= new Date(); });
    setItems(data);
  ...
  function handleFilterChange(key, val) { setFilters(function(prev) { var n = Object.assign({}, prev); n[key] = val; return n; }); }

  function handleToggleTag(key, tag) {
    setFilters(function(prev) {
      var current = prev[key] || [];
      var next = current.includes(tag) ? current.filter(function(t) { return t !== tag; }) : current.concat(tag);
      var n = Object.assign({}, prev); n[key] = next; return n;
    });
  }
  ...
  var activeCount = (
    ...
    filters.causeAreas.length + filters.audience.length + filters.languages.length +
    (keyword ? 1 : 0)
  );

  var filtered = items.filter(function(item) {
    var kw = debouncedKeyword.toLowerCase();
    var matchSearch = !kw || item.title.toLowerCase().includes(kw) || (item.description && item.description.toLowerCase().includes(kw)) || (item.eligibility && item.eligibility.toLowerCase().includes(kw)) || (orgs[item.organization_id] && orgs[item.organization_id].toLowerCase().includes(kw)) || (item.tags && item.tags.some(function(t) { return t.toLowerCase().includes(kw); }));
  ...
  return (
    <>
        ...
        <div style={{ background: pageBg, borderBottom: '1px solid ' + borderColor, position: 'sticky', top: 64, zIndex: 30 }}>
          <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '12px 16px', display: 'flex', alignItems: 'center', gap: '12px', flexWrap: 'wrap' }}>
            ...
            <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
              ...
              <input ref={searchRef} type="search" value={keyword} onChange={function(e) { setKeyword(e.target.value); }} placeholder="Search scholarships, grants, organizations..." aria-label="Search funding opportunities" style={{ width: '100%', paddingLeft: '36px', paddingRight: keyword ? '36px' : '16px', paddingTop: '8px', paddingBottom: '8px', background: cardBg, border: '1px solid ' + borderColor, borderRadius: '8px', fontSize: '14px', color: textPrimary, outline: 'none', boxSizing: 'border-box' }} className="focus:ring-2 focus:ring-blue-500" />
              {keyword && <button onClick={function() { setKeyword(''); }} style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', color: textMuted, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', padding: '2px' }} aria-label="Clear search" className="focus:outline-none focus:ring-2 focus:ring-blue-500 rounded"><X size={14} /></button>}
            </div>
            <button onClick={function() { setMobileFiltersOpen(true); }} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px 12px', background: activeCount > 0 ? '#EFF6FF' : cardBg, border: '1px solid ' + (activeCount > 0 ? '#3B82F6' : borderColor), borderRadius: '8px', fontSize: '13px', color: activeCount > 0 ? '#3B82F6' : textSecondary, cursor: 'pointer', whiteSpace: 'nowrap' }} className="lg:hidden focus:outline-none focus:ring-2 focus:ring-blue-500" aria-label={'Open filters' + (activeCount > 0 ? ', ' + activeCount + ' active' : '')} aria-expanded={mobileFiltersOpen}>
              <SlidersHorizontal size={14} aria-hidden="true" />Filters
              {activeCount > 0 && <span style={{ background: '#3B82F6', color: '#fff', borderRadius: '99px', fontSize: '11px', fontWeight: 700, padding: '0px 6px', lineHeight: '18px' }} aria-hidden="true">{activeCount}</span>}
            </button>
          </div>
        </div>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '24px 16px' }}>
          <div style={{ display: 'flex', gap: '24px' }}>
              ...
              <div className="fixed inset-0 z-50 lg:hidden" role="dialog" aria-modal="true" aria-label="Filters">
                ...
                <div style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: '320px', maxWidth: '100%', background: cardBg, boxShadow: '4px 0 24px rgba(0,0,0,0.15)', overflowY: 'auto', padding: '16px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                    ...
                    <button onClick={function() { setMobileFiltersOpen(false); }} style={{ padding: '6px', borderRadius: '8px', color: textMuted, background: 'none', border: 'none', cursor: 'pointer' }} aria-label="Close filters" className="focus:outline-none focus:ring-2 focus:ring-blue-500"><X size={18} /></button>
                  </div>
                  <SidebarFilters filters={filters} onChange={handleFilterChange} onToggleTag={handleToggleTag} onReset={handleReset} activeCount={activeCount} tagSets={tagSets} />
            ...
            <main style={{ flex: 1, minWidth: 0 }} aria-label="Funding listings">
                    ...
                    {chips.map(function(chip) {
                      return (
                        <span key={chip.key} role="listitem" style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#F1F5F9', border: '1px solid #E2E8F0', color: '#374151', borderRadius: '99px', padding: '3px 10px', fontSize: '11px', fontWeight: 600 }}>
                          {chip.label}
                          <button onClick={chip.clear} aria-label={'Remove filter: ' + chip.label} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#64748B', display: 'flex', alignItems: 'center', padding: '0' }} className="focus:outline-none focus:ring-1 focus:ring-blue-400 rounded-full"><X size={10} aria-hidden="true" /></button>
                        </span>
                      );
                    })}
              ...
              {loading && <div aria-label="Loading funding" aria-busy="true">{[1,2,3,4,5,6].map(function(i) { return <FundingCardSkeleton key={i} />; })}</div>}

              {!loading && error && (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '80px 0', textAlign: 'center' }} role="alert">
                  <div style={{ width: '64px', height: '64px', borderRadius: '16px', background: '#FEF2F2', border: '1px solid #FECACA', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '16px' }}><AlertCircle size={28} color="#EF4444" aria-hidden="true" /></div>
      ...
      {selected && <DetailDrawer item={selected} orgName={orgs[selected.organization_id] || 'Verified Nonprofit'} onClose={function() { setSelected(null); }} onApply={function() { setApplying(selected); }} />}
      {applying && <ApplyModal item={applying} currentUser={currentUser} onClose={function() { setApplying(null); }} />}
    </>
  );
}