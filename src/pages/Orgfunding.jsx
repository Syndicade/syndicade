...
import {
  ...
  AlertCircle, CheckCircle, Search, Paperclip, Upload,
  Inbox, Tag, Sparkles
} from 'lucide-react';
    ...
    <div aria-busy="true" aria-label="Loading funding listings">
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '12px' }}>
        {[1, 2, 3].map(function(i) {
          return (
            <div key={i} style={{ background: cardBg, border: '0.5px solid ' + borderColor, borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }} className="animate-pulse">
    ...
    return (
      <div style={{ textAlign: 'center', padding: '60px 24px' }}>
        ...
        <p style={{ fontSize: '14px', color: textSecondary, maxWidth: '360px', margin: '0 auto 20px', lineHeight: 1.6 }}>
          Posting grants and scholarships is available to verified 501(c)(3) organizations. Get verified to share funding opportunities with the community.
        </p>
...
function FundingModal({ organizationId, currentUserId, existing, onClose, onSaved, templateBanner }) {
  ...
  var [tagGroups, setTagGroups]     = useState({ causeAreas: [], audience: [], fundingTypes: [], languages: [] });
  ...
  var detailsHasError  = !!(errors.title || errors.description);
  var settingsHasError = !!(errors.apply_url || errors.amount_min);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '24px 16px', zIndex: 50, overflowY: 'auto' }}
      ...
      <div style={{ background: cardBg, borderRadius: '16px', width: '100%', maxWidth: '600px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)', marginTop: '16px', marginBottom: '24px' }}>
                ...
                {form.amount_type === 'fixed' && (
                  <div style={{ position: 'relative' }}>
                    ...
                    <input id="fund-amount-fixed" type="number" min="0" value={form.amount_min} onChange={function(e) { setField('amount_min', e.target.value); }}
                      ...
                      aria-label="Fixed amount" className="focus:ring-2 focus:ring-blue-500" />
                    {errors.amount_min && <p style={errorStyle} role="alert"><AlertCircle size={11} aria-hidden="true" />{errors.amount_min}</p>}
                  </div>
                )}
        ...
        {/* Footer — blue primary button throughout */}
          ...
          <div>
            {!existing && (
              <button type="button" onClick={function() { handleSave('draft'); }} disabled={saving}
                ...
                className="hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400">Save as Draft</button>
            )}
          </div>
...
// ── Funding card ──────────────────────────────────────────────────────────────
function FundingCard({ item, appCount, onEdit, onDelete, onDuplicate, onMakeTemplate, onViewApps }) {
  ...
  return (
    <article style={{ background: cardBg, border: '0.5px solid ' + (isExpired ? '#FECACA' : borderColor), borderRadius: '12px', padding: '16px', display: 'flex', flexDirection: 'column', gap: '10px', height: '100%', boxSizing: 'border-box' }}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
        {formatAmount() && <p style={{ fontSize: '12px', color: '#16A34A', fontWeight: 700, margin: 0 }}>{formatAmount()}</p>}
        {item.deadline && (
...
function ConfirmDeleteModal({ item, onConfirm, onCancel }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 60 }}
      role="dialog" aria-modal="true" aria-labelledby="confirm-del-fund-title"
      ...
      <div style={{ background: cardBg, borderRadius: '14px', padding: '28px', maxWidth: '380px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        ...
        <h3 id="confirm-del-fund-title" style={{ fontSize: '16px', fontWeight: 800, color: textPrimary, textAlign: 'center', marginBottom: '8px' }}>Delete Listing?</h3>
...
function MakeTemplateModal({ item, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    var result = await supabase.from('org_funding').insert(payload);
    setSaving(false);
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px', zIndex: 60 }}
      role="dialog" aria-modal="true" aria-labelledby="tmpl-fund-title"
      onClick={function(e) { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ background: cardBg, borderRadius: '14px', padding: '28px', maxWidth: '400px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <h3 id="tmpl-fund-title" style={{ fontSize: '16px', fontWeight: 800, color: textPrimary, marginBottom: '6px' }}>Save as Template</h3>
        <p style={{ fontSize: '13px', color: textMuted, marginBottom: '20px' }}>This funding listing will be saved as a reusable template for your org.</p>
        <label htmlFor="tmpl-fund-name" style={{ fontSize: '13px', fontWeight: 600, color: textPrimary, display: 'block', marginBottom: '6px' }}>Template name</label>
        <input id="tmpl-fund-name" value={name} onChange={function(e) { setName(e.target.value); }}
...
function ApplicationsDrawer({ item, onClose }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', justifyContent: 'flex-end', zIndex: 50 }}
      role="dialog" aria-modal="true" aria-labelledby="fund-apps-drawer-title"
      onClick={function(e) { if (e.target === e.currentTarget) onClose(); }}>
      <div style={{ width: '100%', maxWidth: '460px', background: cardBg, height: '100%', overflowY: 'auto', boxShadow: '-4px 0 24px rgba(0,0,0,0.1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '20px 24px', borderBottom: '1px solid ' + borderColor, position: 'sticky', top: 0, background: cardBg, zIndex: 1 }}>
          <div>
            <h2 id="fund-apps-drawer-title" style={{ fontSize: '15px', fontWeight: 800, color: textPrimary, margin: 0 }}>Applications</h2>
...
function OrgFunding() {
  ...
  async function loadItems() {
    setLoading(true);
    var query = supabase.from('org_funding').select('*')
    ...
    if (formIds.length > 0) {
      var countResult = await supabase.from('funding_applications').select('funding_id').in('funding_id', formIds);
      var counts = {};
      (countResult.data || []).forEach(function(row) { counts[row.funding_id] = (counts[row.funding_id] || 0) + 1; });
      setAppCounts(counts);
    }
    setLoading(false);
  }
...
function handleTemplateSelect(template, name) {
  ...
  async function handleDuplicate(item) {
    ...
    mascotSuccessToast('Listing duplicated — saved as draft.');
    loadItems();
  }
  ...
  return (
    <div style={{ background: pageBg, minHeight: '100vh' }}>
          ...
          {items.length > 0 && (
            <div style={{ display: 'flex', gap: '10px', marginBottom: '16px', flexWrap: 'wrap', alignItems: 'center' }}>
              <div style={{ position: 'relative', flex: 1, minWidth: '200px' }}>
                ...
                <input value={search} onChange={function(e) { setSearch(e.target.value); }} placeholder="Search funding listings..."
                  ...
                  aria-label="Search listings" className="focus:ring-2 focus:ring-blue-500" />
              </div>
              ...
              {filtered.map(function(item) {
                return (
                  <div key={item.id} role="listitem">
                    <FundingCard item={item} appCount={appCounts[item.id] || 0}
                      onEdit={openEdit}
      ...
      {showModal && (
        <FundingModal organizationId={organizationId} currentUserId={currentUserId} existing={editing}
