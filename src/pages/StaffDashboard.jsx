 ...
 * 2. Members    — search, view, account tools (reset password, suspend)
      ...
      <div className="relative w-full max-w-md bg-[#151B2D] border-l border-[#2A3550] h-full overflow-y-auto flex flex-col">
        ...
        <div className="flex-1 px-6 py-5 space-y-6">
          ...
          <div className="bg-[#1A2035] border border-[#2A3550] rounded-xl p-4 space-y-2">
            <div className="flex justify-between text-[13px]"><span className="text-[#64748B]">Joined</span><span className="text-[#CBD5E1]">{new Date(member.created_at).toLocaleDateString()}</span></div>
...
function MembersTab() {
  ...
  async function handleSearch(e) {
    ...
    var { data, error } = await supabase.from('members')
      .select('user_id, first_name, last_name, email, phone, created_at, is_staff, account_status')
      .or('first_name.ilike.%' + query + '%,last_name.ilike.%' + query + '%,email.ilike.%' + query + '%')
      .limit(25);
    if (error) { toast.error('Search failed.'); } else { setResults(data || []); }
    setLoading(false);
  }
    ...
    <div>
      <form onSubmit={handleSearch} className="flex gap-3 mb-6" role="search" aria-label="Search members">
        <div className="relative flex-1">
          ...
          <input id="member-search" type="search" value={query} onChange={function (e) { setQuery(e.target.value); }} placeholder="Search by name or email..." className="w-full bg-[#1A2035] border border-[#2A3550] text-[#CBD5E1] text-[14px] rounded-xl pl-9 pr-4 py-3 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 placeholder-[#64748B]" autoComplete="off" />
        </div>
...
function OverviewTab() {
  ...
  <button
    onClick={function () {
      var path = v.document_url.split('/verification-docs/')[1];
      getSignedUrl(path);
    }}
    className="text-[12px] text-blue-400 hover:text-blue-300 underline focus:outline-none focus:ring-2 focus:ring-blue-500 rounded flex-shrink-0"
    aria-label={'View IRS letter for ' + orgName}
  >
    View Document
  </button>
)}
