...
var SURVEY_TEMPLATES = (PLATFORM_TEMPLATES['survey'] || []);
...
function SurveysList() {
  ...
  useEffect(function() { setSelectedIds(new Set()); }, [searchTerm, statusFilter, sortBy]);

  function isSurveyClosed(s) {
    return s.status === 'closed' || (s.closes_at && new Date(s.closes_at) < new Date());
  }
  ...
  async function loadData() {
    try {
      ...
      if (count > 0 && surveyList.length > 0) {
        var activeSurveyIds = surveyList.filter(function(s) { return !isSurveyClosed(s); }).map(function(s) { return s.id; });
        if (activeSurveyIds.length > 0) {
          var respResult = await supabase.from('survey_responses').select('survey_id').in('survey_id', activeSurveyIds);
          if (!respResult.error && respResult.data) {
            var countsBySurvey = {};
            respResult.data.forEach(function(r) { countsBySurvey[r.survey_id] = (countsBySurvey[r.survey_id] || 0) + 1; });
            var rates = activeSurveyIds.map(function(id) { return Math.min(100, Math.round(((countsBySurvey[id] || 0) / count) * 100)); });
            setAvgResponseRate(rates.length > 0 ? Math.round(rates.reduce(function(s, r) { return s + r; }, 0) / rates.length) : 0);
          }
        } else { setAvgResponseRate(null); }
      }
    } catch (err) {
  ...
  async function handleExport() {
    ...
    try {
      ...
      var respResult = await supabase.from('survey_responses').select('id, survey_id').in('survey_id', surveyIds);
      if (respResult.error) throw respResult.error;
      var allResponses = respResult.data || [];
      var responseIdsBySurvey = {};
      allResponses.forEach(function(r) { if (!responseIdsBySurvey[r.survey_id]) responseIdsBySurvey[r.survey_id] = []; responseIdsBySurvey[r.survey_id].push(r.id); });
      var allResponseIds = allResponses.map(function(r) { return r.id; });
      var answersByResponseId = {};
      if (allResponseIds.length > 0) {
        var ansResult = await supabase.from('survey_answers').select('response_id, question_id, answer_text').in('response_id', allResponseIds);
        if (!ansResult.error && ansResult.data) {
          ansResult.data.forEach(function(a) { if (!answersByResponseId[a.response_id]) answersByResponseId[a.response_id] = []; answersByResponseId[a.response_id].push(a); });
        }
      }
      var qResult = await supabase.from('survey_questions').select('id, survey_id, question_text, question_type, options, order_number').in('survey_id', surveyIds).order('order_number');
      if (qResult.error) throw qResult.error;
      var questionsBySurvey = {};
      (qResult.data || []).forEach(function(q) { if (!questionsBySurvey[q.survey_id]) questionsBySurvey[q.survey_id] = []; questionsBySurvey[q.survey_id].push(q); });
      ...
      surveysToExport.forEach(function(survey) {
        ...
        if (questions.length === 0) { rows.push([survey.title, statusLabel, '(no questions)', '', '', '', '', totalResponses]); rows.push(['','','','','','','','']); return; }
        questions.forEach(function(q, qi) {
          var allAnswersForQ = [];
          surveyResponses.forEach(function(respId) { var ans = (answersByResponseId[respId] || []).find(function(a) { return a.question_id === q.id; }); if (ans) allAnswersForQ.push(ans.answer_text); });
          var surveyCol = qi === 0 ? survey.title : '';
          var statusCol = qi === 0 ? statusLabel : '';
          var totalCol  = qi === 0 ? totalResponses : '';
          if (q.question_type === 'single_choice' || q.question_type === 'multiple_choice') {
            var opts = q.options || []; var counts = {};
            opts.forEach(function(o) { counts[o] = 0; });
            allAnswersForQ.forEach(function(at) {
              if (q.question_type === 'multiple_choice') { try { JSON.parse(at || '[]').forEach(function(o) { if (counts.hasOwnProperty(o)) counts[o]++; }); } catch(e) {} }
              else { if (counts.hasOwnProperty(at)) counts[at]++; }
            });
            opts.forEach(function(opt, oi) { var c = counts[opt] || 0; var pct = totalResponses > 0 ? Math.round((c / totalResponses) * 100) : 0; rows.push([oi===0?surveyCol:'', oi===0?statusCol:'', oi===0?q.question_text:'', oi===0?q.question_type:'', opt, c, pct+'%', oi===0?totalCol:'']); });
            ...
            if (allAnswersForQ.length === 0) { rows.push([surveyCol, statusCol, q.question_text, q.question_type, '(no responses)', '', '', totalCol]); }
            else { allAnswersForQ.forEach(function(at, ai) { rows.push([ai===0?surveyCol:'', ai===0?statusCol:'', ai===0?q.question_text:'', ai===0?q.question_type:'', at||'', '', '', ai===0?totalCol:'']); }); }
          }
        });
        rows.push(['','','','','','','','']);
      });
  ...
  function handleSurveyUpdated(updatedSurvey) {
    setSurveys(function(prev) { return prev.map(function(s) { return s.id === updatedSurvey.id ? updatedSurvey : s; }); });
  }
  ...
  function openCreate() { setEditingSurvey(null); setTemplateData(null); setShowCreateModal(true); }
  function openEdit(survey) { setEditingSurvey(survey); setTemplateData(null); setShowCreateModal(true); }
  function closeModal() { setShowCreateModal(false); setEditingSurvey(null); setTemplateData(null); }
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="px-6 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <div className="rounded-xl p-5 border bg-[#DBEAFE] border-blue-200">
            <p className="text-2xl font-extrabold text-blue-700">
              {avgResponseRate !== null ? avgResponseRate + '%' : '\u2014'}
            </p>
            <div className="flex items-center gap-2 mt-1">
              <Icon path={ICONS.users} className="h-4 w-4 text-blue-500" />
              <p className="text-sm font-semibold text-blue-600">Avg Response</p>
            </div>
          </div>
        </div>
        ...
        {/* Survey list or empty states */}
        {surveys.length === 0 ? (
          /* No surveys at all — mascot + template cards */
          <div className="rounded-xl border bg-white border-slate-200 py-12 px-6 text-center">
            ...
            <h3 className="text-lg font-bold mb-2 text-[#0E1523]">No surveys yet</h3>
            <p className="text-sm mb-6 text-[#475569] max-w-xs mx-auto">
              {isAdmin
                ? 'Create your first survey to gather structured feedback from members.'
                : 'Check back soon for surveys from your organization.'}
            </p>
            {isAdmin && (
            ...
            {isAdmin && SURVEY_TEMPLATES.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-w-xl mx-auto text-left">
                  {SURVEY_TEMPLATES.map(function(tmpl) {
                    return (
                ...
                <div key={survey.id} role="listitem">
                  {isAdmin && (
                    <div className={'flex items-center gap-2 px-3 py-1.5 rounded-t-xl border-x border-t transition-colors ' + (isSelected ? 'bg-blue-50 border-blue-200' : 'bg-slate-50 border-slate-200')}>
                      <input id={'select-survey-' + survey.id} type="checkbox" checked={isSelected}
                  ...
                  <div className={isAdmin ? 'rounded-b-xl overflow-hidden' + (isSelected ? ' ring-2 ring-blue-300' : '') : ''}>
                    <SurveyCard
                      ...
                      onRemind={function(surveyId, timestamp) {
                        setSurveys(function(prev) { return prev.map(function(s) { return s.id === surveyId ? Object.assign({}, s, { last_reminded_at: timestamp }) : s; }); });
                      }}
                      isAdmin={isAdmin}
