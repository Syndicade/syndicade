...
function TemplatePicker({ organizationId, onLoad, onClose }) {
  ...
  async function fetchTemplates() {
    setLoading(true);
    try {
      var r = await supabase.from('poll_survey_templates').select('id, name, template_data, created_at').eq('organization_id', organizationId).eq('type', 'survey').order('created_at', { ascending: false });
  ...
  async function saveRename(tmplId) {
    ...
    try {
      var r = await supabase.from('poll_survey_templates').update({ name: trimmed }).eq('id', tmplId);
      if (r.error) throw r.error;
      setTemplates(function(prev) { return prev.map(function(t) { return t.id === tmplId ? Object.assign({}, t, { name: trimmed }) : t; }); });
      setRenamingId(null);
      mascotSuccessToast('Template renamed.');
    } catch (err) { mascotErrorToast('Failed to rename template.', err.message); }
  ...
  async function confirmDeleteTemplate() {
    ...
    setConfirmDelete(null); setDeletingId(tmplId);
    try {
      var r = await supabase.from('poll_survey_templates').delete().eq('id', tmplId);
      if (r.error) throw r.error;
      ...
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-[60]" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="template-picker-title">
        <div ref={modalRef} className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[70vh] flex flex-col" onClick={function(e){e.stopPropagation();}}>
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-200 flex-shrink-0">
            ...
            <button type="button" onClick={onClose} className="p-1.5 text-[#64748B] hover:text-[#0E1523] hover:bg-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" aria-label="Close template picker">
              <Icon path={ICONS.x} className="h-4 w-4" />
            </button>
          </div>
          <div className="overflow-y-auto flex-1 p-4 space-y-2">
            {loading ? (
              [1,2,3].map(function(i) { return (<div key={i} className="border border-slate-200 rounded-lg p-4 animate-pulse"><div className="h-4 w-40 bg-gray-200 rounded mb-2"/><div className="h-3 w-24 bg-gray-100 rounded"/></div>); })
            ) : templates.length === 0 ? (
              <div className="text-center py-10">
                <div className="w-12 h-12 bg-slate-100 rounded-full flex items-center justify-center mx-auto mb-3"><Icon path={ICONS.template} className="h-6 w-6 text-[#94A3B8]"/></div>
              ...
              return (
                <div key={tmpl.id} className="border border-slate-200 rounded-lg p-4 hover:border-blue-300 hover:bg-blue-50 transition-colors">
                  {renamingId === tmpl.id ? (
                    <div className="flex items-center gap-2">
                      <input type="text" value={renameValue} onChange={function(e){setRenameValue(e.target.value);}} onKeyDown={function(e){if(e.key==='Enter')saveRename(tmpl.id);if(e.key==='Escape')setRenamingId(null);}} className="flex-1 px-3 py-1.5 border border-blue-400 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 text-[#0E1523]" autoFocus maxLength={100}/>
                      <button type="button" onClick={function(){saveRename(tmpl.id);}} disabled={renameSaving} className="p-1.5 bg-blue-500 text-white rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50" aria-label="Save rename"><Icon path={ICONS.check} className="h-4 w-4"/></button>
                      <button type="button" onClick={function(){setRenamingId(null);}} className="p-1.5 text-[#64748B] hover:bg-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400" aria-label="Cancel rename"><Icon path={ICONS.x} className="h-4 w-4"/></button>
                    </div>
                  ) : (
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button type="button" onClick={function(){onLoad(tmpl);}} className="px-3 py-1.5 bg-blue-500 text-white text-xs font-semibold rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors">Load</button>
                        <button type="button" onClick={function(){setRenamingId(tmpl.id);setRenameValue(tmpl.name);}} className="p-1.5 text-[#64748B] hover:text-[#0E1523] hover:bg-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-400 transition-colors" aria-label={'Rename: '+tmpl.name}><Icon path={ICONS.pencil} className="h-4 w-4"/></button>
                        <button type="button" onClick={function(){setConfirmDelete({id:tmpl.id,name:tmpl.name});}} disabled={isDeleting} className="p-1.5 text-[#94A3B8] hover:text-red-500 hover:bg-red-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400 disabled:opacity-50 transition-colors" aria-label={'Delete: '+tmpl.name}><Icon path={ICONS.trash} className="h-4 w-4"/></button>
                      </div>
                    </div>
                  )}
                </div>
...
// ─── CreateSurvey ─────────────────────────────────────────────────────────────
function CreateSurvey({ isOpen, onClose, onSuccess, organizationId, organizationName, editSurvey, templateData }) {
  // ── isEditMode must be declared before any useEffect that references it ──
  ...
  // Handle open/edit/template state
  useEffect(function() {
    if (!isOpen) {
      setHasResponses(false);
      return;
    }
    if (isEditMode && editSurvey) {
      loadEditData();
    } else if (!isEditMode && templateData && templateData._questions) {
      // Platform template — _questions array already in correct shape
      setFormData({
        title:                         templateData.title || '',
        description:                   templateData.description || '',
        anonymous_responses:           !!templateData.anonymous_responses,
        allow_multiple_responses:      !!templateData.allow_multiple_responses,
        show_results_after_submission: templateData.show_results_after_submission !== false,
        result_visibility:             templateData.result_visibility || 'full',
        visibility:                    templateData.visibility || 'all_members',
        closes_at: '', retention_days: '', is_recurring: false,
        recurring_interval: 'monthly', recurring_ends_at: '',
      });
      setQuestions(templateData._questions.map(function(tq, i) {
        return makeQuestionFromTemplate(tq, i + 1);
      }));
    } else if (!isEditMode && templateData && templateData.template_data) {
      // Org-saved template from poll_survey_templates table
      var data = templateData.template_data;
      setFormData({
        title:                         data.title || '',
        description:                   data.description || '',
        anonymous_responses:           !!data.anonymous_responses,
        allow_multiple_responses:      !!data.allow_multiple_responses,
        show_results_after_submission: data.show_results_after_submission !== false,
        result_visibility:             data.result_visibility || 'full',
        visibility:                    data.visibility || 'all_members',
        closes_at: '', retention_days: '', is_recurring: false,
        recurring_interval: 'monthly', recurring_ends_at: '',
      });
      setQuestions(Array.isArray(data.questions) && data.questions.length > 0
        ? data.questions.map(function(tq, i) { return makeQuestionFromTemplate(tq, i + 1); })
        : [makeQuestion(1)]
      );
  ...
  async function loadOrgRoles() {
    ...
    try {
      var r = await supabase.from('membership_tiers').select('id, name').eq('organization_id', organizationId).order('name', { ascending: true });
  ...
  async function loadEditData() {
    ...
    try {
      var s = editSurvey;
      var closesAtValue = '';
      if (s.closes_at) { closesAtValue = new Date(s.closes_at).toISOString().slice(0, 16); }
      setFormData({
        title:                         s.title || '',
      ...
      var loadedQs = (qR.data || []).map(function(q, i) {
        return {
          id: Date.now() + Math.random() + i, _dbId: q.id,
          question_text: q.question_text || '', question_type: q.question_type || 'text',
          required: q.required !== false, options: q.options || [],
          order_number: q.order_number || i + 1, has_condition: !!(q.condition_question_id),
          condition_question_id: null, condition_answer: q.condition_answer || '',
    ...
    } catch (err) { console.error('Error loading survey for edit:', err); setError('Failed to load survey data.'); }
  ...
  // Internal "Load Template" picker (org-saved templates)
  ...
  function addQuestion() { setQuestions(function(prev) { return prev.concat([makeQuestion(prev.length + 1)]); }); }
  function removeQuestion(qId) {
    if (questions.length === 1) { setError('Survey must have at least one question'); return; }
    setQuestions(function(prev) { return prev.filter(function(q) { return q.id !== qId; }).map(function(q, i) { return Object.assign({}, q, { order_number: i + 1 }); }); });
    setError(null);
  }
  function updateQuestion(qId, field, value) { setQuestions(function(prev) { return prev.map(function(q) { return q.id === qId ? Object.assign({}, q, { [field]: value }) : q; }); }); }
  ...
  function addOption(qId) { setQuestions(function(prev) { return prev.map(function(q) { return q.id === qId ? Object.assign({}, q, { options: (q.options||[]).concat(['']) }) : q; }); }); }
  function updateOption(qId, oi, value) { setQuestions(function(prev) { return prev.map(function(q) { if (q.id !== qId) return q; var opts = q.options.slice(); opts[oi] = value; return Object.assign({}, q, { options: opts }); }); }); }
  function removeOption(qId, oi) { setQuestions(function(prev) { return prev.map(function(q) { return q.id !== qId ? q : Object.assign({}, q, { options: q.options.filter(function(_, i) { return i !== oi; }) }); }); }); }
  ...
  function getConditionableQuestions(currentQId) {
    ...
    return questions.slice(0, idx).filter(function(q) { return ['single_choice','multiple_choice','yes_no_abstain','rating'].includes(q.question_type) && q.question_text.trim(); });
  }
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      var surveyPayload = {
        ...
        visibility: formData.visibility, result_visibility: formData.result_visibility,
  ...
  return (
    <>
        ...
        <div ref={modalRef} className="bg-white rounded-xl shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col" onClick={function(e){e.stopPropagation();}}>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 id="create-survey-title" className="text-xl font-bold text-[#0E1523]">{isEditMode ? 'Edit Survey' : 'Create Survey'}</h2>
                <p className="text-[#64748B] text-sm">{organizationName}</p>
              </div>
            </div>
            <div className="flex items-center gap-2">
              {!isEditMode && (
                <button type="button" onClick={function(){setShowTemplatePicker(true);}} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-semibold text-[#475569] bg-slate-100 hover:bg-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" aria-label="Load a saved template">
                  <Icon path={ICONS.template} className="h-4 w-4"/>Load Template
                </button>
              )}
              <button type="button" onClick={handleClose} className="p-2 text-[#64748B] hover:text-[#0E1523] hover:bg-slate-100 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors" aria-label="Close dialog">
          ...
          <div className="overflow-y-auto flex-1 px-6 py-6 space-y-6">
            {editLoading ? (
              <div className="space-y-4 animate-pulse">{[1,2,3,4,5].map(function(i){return <div key={i} className="h-10 rounded-lg bg-gray-100"/>;})}</div>
            ) : (
              <>
                ...
                {/* Template banner */}
                {!isEditMode && templateData && templateData._templateName && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                    <p className="text-sm text-blue-700 font-semibold">Started from template: {templateData._templateName} — feel free to edit anything.</p>
                  </div>
                )}
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <input id="survey-title" type="text" value={formData.title} onChange={function(e){updateForm('title',e.target.value);}} maxLength={200} className={inputCls} placeholder="e.g., Annual Member Satisfaction Survey" required aria-required="true"/>
                  ...
                  <div>
                    ...
                    <textarea id="survey-description" value={formData.description} onChange={function(e){updateForm('description',e.target.value);}} rows={3} maxLength={1000} className={inputCls+' resize-none'} placeholder="What is this survey about?"/>
                    <p className="text-xs text-[#94A3B8] mt-1" aria-live="polite">{formData.description.length}/1000</p>
                  </div>

                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="survey-closing" className={labelCls}><span className="flex items-center gap-2"><Icon path={ICONS.clock} className="h-4 w-4 text-[#94A3B8]"/>Closing Date <span className="text-[#94A3B8] font-normal">(optional)</span></span></label>
                      <input id="survey-closing" type="datetime-local" value={formData.closes_at} onChange={function(e){updateForm('closes_at',e.target.value);}} className={inputCls}/>
                      <p className="text-xs text-[#94A3B8] mt-1">Survey closes automatically at this time</p>
                    </div>
                    <div>
                      <label htmlFor="survey-visibility" className={labelCls}><span className="flex items-center gap-2"><Icon path={ICONS.users} className="h-4 w-4 text-[#94A3B8]"/>Who Can Respond</span></label>
                      {rolesLoading ? <div className="h-12 bg-slate-100 rounded-lg animate-pulse" aria-hidden="true"/> : (
                        <select id="survey-visibility" value={formData.visibility} onChange={function(e){updateForm('visibility',e.target.value);}} className={inputCls}>
                  ...
                  <div>
                    <p className={labelCls} id="result-vis-label"><span className="flex items-center gap-2"><Icon path={ICONS.eye} className="h-4 w-4 text-[#94A3B8]"/>Result Visibility</span></p>
                    <div className="space-y-2" role="radiogroup" aria-labelledby="result-vis-label">
                      {RESULT_VISIBILITY_OPTIONS.map(function(opt){
                        ...
                        return(<label key={opt.value} className={'flex items-start p-3 border-2 rounded-xl cursor-pointer transition-colors '+(checked?'border-blue-500 bg-blue-50':'border-slate-200 hover:bg-slate-50')}>
                          <input type="radio" name="result_visibility" value={opt.value} checked={checked} onChange={function(e){updateForm('result_visibility',e.target.value);}} className="w-4 h-4 text-blue-600 focus:ring-blue-500 mt-0.5 flex-shrink-0"/>
                          <div className="ml-3"><p className="font-semibold text-[#0E1523] text-sm">{opt.label}</p><p className="text-xs text-[#64748B]">{opt.desc}</p></div>
                        </label>);
                      })}
                    </div>
                  </div>
                  ...
                  <div>
                    <label htmlFor="survey-retention" className={labelCls}><span className="flex items-center gap-2"><Icon path={ICONS.database} className="h-4 w-4 text-[#94A3B8]"/>Data Retention</span></label>
                    <select id="survey-retention" value={formData.retention_days} onChange={function(e){updateForm('retention_days',e.target.value);}} className={inputCls}>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        return(<label key={item.id} className={'flex items-start gap-3 p-3 rounded-xl border cursor-pointer transition-colors '+(item.checked?'border-blue-300 bg-white':'border-transparent hover:bg-white')}>
                          <input id={'survey-'+item.id} type="checkbox" checked={item.checked} onChange={function(e){updateForm(item.field,e.target.checked);}} className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 mt-0.5 flex-shrink-0"/>
                          <div className="flex-1 min-w-0"><div className="flex items-center gap-2"><Icon path={item.icon} className={'h-4 w-4 flex-shrink-0 '+item.color}/><span className="font-semibold text-[#0E1523] text-sm">{item.label}</span></div><p className="text-xs text-[#64748B] mt-0.5">{item.desc}</p></div>
                        </label>);
                      })}
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-5">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2"><Icon path={ICONS.repeat} className="h-4 w-4 text-[#94A3B8]"/><span className="text-sm font-semibold text-[#0E1523]">Recurring Survey</span></div>
                      <label className="relative inline-flex items-center cursor-pointer" aria-label="Toggle recurring">
                        <input type="checkbox" checked={formData.is_recurring} onChange={function(e){updateForm('is_recurring',e.target.checked);}} className="sr-only peer"/>
                      ...
                      <div className="space-y-3 pt-3 border-t border-slate-200">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <select id="recurring-interval" value={formData.recurring_interval} onChange={function(e){updateForm('recurring_interval',e.target.value);}} className={inputCls}>
                          ...
                          <div>
                            ...
                            <input id="recurring-ends" type="date" value={formData.recurring_ends_at} onChange={function(e){updateForm('recurring_ends_at',e.target.value);}} className={inputCls}/>
                          </div>
                        </div>
                        <p className="text-xs text-blue-600 bg-blue-50 border border-blue-100 rounded-lg px-3 py-2">A new survey will be created automatically when the current one closes.</p>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="border-t border-slate-200 pt-6 space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button type="button" onClick={addQuestion} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-500 text-white font-semibold text-sm rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-colors">
                        ...
                        <div key={question.id} className={'border border-slate-200 rounded-xl p-5 space-y-4 bg-white '+(question.has_condition?'border-l-4 border-l-blue-300':'')}>
                          <div className="flex items-center justify-between">
                            <span className="text-xs font-bold text-[#64748B] uppercase tracking-widest">
                              {'Question '+(index+1)}{question.has_condition&&<span className="ml-2 text-blue-500 normal-case font-semibold">· conditional</span>}
                            </span>
                            <div className="flex items-center gap-1">
                              <button type="button" onClick={function(){moveQuestion(question.id,'up');}} disabled={index===0} className="p-1.5 text-[#94A3B8] hover:text-[#475569] hover:bg-slate-100 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-400 transition-colors" aria-label="Move question up"><Icon path={ICONS.chevUp} className="h-4 w-4"/></button>
                              <button type="button" onClick={function(){moveQuestion(question.id,'down');}} disabled={index===questions.length-1} className="p-1.5 text-[#94A3B8] hover:text-[#475569] hover:bg-slate-100 rounded-lg disabled:opacity-30 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-gray-400 transition-colors" aria-label="Move question down"><Icon path={ICONS.chevDown} className="h-4 w-4"/></button>
                              <button type="button" onClick={function(){removeQuestion(question.id);}} className="p-1.5 text-[#94A3B8] hover:text-red-500 hover:bg-red-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400 transition-colors ml-1" aria-label={'Delete question '+(index+1)}><Icon path={ICONS.trash} className="h-4 w-4"/></button>
                            </div>
                          </div>
                          <div>
                            <label htmlFor={'q-text-'+question.id} className="sr-only">{'Question '+(index+1)+' text'}</label>
                            <input id={'q-text-'+question.id} type="text" value={question.question_text} onChange={function(e){updateQuestion(question.id,'question_text',e.target.value);}} className={inputCls} placeholder="Enter your question" required aria-required="true"/>
                          </div>
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label htmlFor={'q-type-'+question.id} className="block text-xs font-semibold text-[#64748B] mb-1">Question Type</label>
                              <select id={'q-type-'+question.id} value={question.question_type} onChange={function(e){updateQuestion(question.id,'question_type',e.target.value);}} className={inputCls}>
                                {QUESTION_TYPES.map(function(t){return <option key={t.value} value={t.value}>{t.label}</option>;})}
                              </select>
                            </div>
                            <div className="flex items-end pb-3">
                              <label className="flex items-center gap-3 cursor-pointer">
                                <input id={'q-req-'+question.id} type="checkbox" checked={question.required} onChange={function(e){updateQuestion(question.id,'required',e.target.checked);}} className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 flex-shrink-0"/>
                                <div><span className="text-sm font-semibold text-[#0E1523]">Required</span><p className="text-xs text-[#94A3B8]">Must be answered</p></div>
                              </label>
                            </div>
                          </div>
                          {isRating && <div className="bg-blue-50 border border-blue-200 rounded-lg p-3"><p className="text-xs text-blue-700 font-medium">Rating scale: 1 (lowest) to 5 (highest) — options are generated automatically.</p></div>}
                          {showOptions && (
                            <div className="space-y-2 pt-1">
                              ...
                              {(question.options||[]).map(function(opt,oi){
                                return(<div key={oi} className="flex items-center gap-2">
                                  ...
                                  <input type="text" value={opt} onChange={function(e){updateOption(question.id,oi,e.target.value);}} className={inputCls} placeholder={'Option '+(oi+1)} aria-label={'Option '+(oi+1)+' for question '+(index+1)}/>
                                  <button type="button" onClick={function(){removeOption(question.id,oi);}} className="p-2 text-[#94A3B8] hover:text-red-500 hover:bg-red-50 rounded-lg focus:outline-none focus:ring-2 focus:ring-red-400 transition-colors flex-shrink-0" aria-label={'Remove option '+(oi+1)}><Icon path={ICONS.x} className="h-4 w-4"/></button>
                                </div>);
                              })}
                              <button type="button" onClick={function(){addOption(question.id);}} className="inline-flex items-center gap-2 px-3 py-2 text-blue-600 hover:bg-blue-50 border border-blue-200 rounded-lg font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors mt-1">
                          ...
                          {index > 0 && conditionableQs.length > 0 && (
                            <div className="pt-3 border-t border-slate-100">
                              <label className="flex items-center gap-3 cursor-pointer mb-3">
                                <input type="checkbox" checked={question.has_condition} onChange={function(e){updateQuestion(question.id,'has_condition',e.target.checked);if(!e.target.checked){updateQuestion(question.id,'condition_question_id',null);updateQuestion(question.id,'condition_answer','');}}} className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500 flex-shrink-0"/>
                                <div className="flex items-center gap-2"><Icon path={ICONS.filter} className="h-4 w-4 text-blue-500"/><span className="text-sm font-semibold text-[#0E1523]">Only show if...</span></div>
                              </label>
                              {question.has_condition && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pl-7">
                                  <div>
                                    ...
                                    <select id={'cond-q-'+question.id} value={question.condition_question_id||''} onChange={function(e){updateQuestion(question.id,'condition_question_id',e.target.value||null);updateQuestion(question.id,'condition_answer','');}} className={inputCls}>
                                      <option value="">Select a question...</option>
                                      {conditionableQs.map(function(cq){return <option key={cq.id} value={cq.id}>{'Q'+(questions.indexOf(cq)+1)+': '+cq.question_text.slice(0,40)+(cq.question_text.length>40?'...':'')}</option>;})}
                                    </select>
                                  </div>
                                  <div>
                                    ...
                                    {question.condition_question_id && condAnswers.length > 0 ? (
                                      <select id={'cond-a-'+question.id} value={question.condition_answer||''} onChange={function(e){updateQuestion(question.id,'condition_answer',e.target.value);}} className={inputCls}>
                                        <option value="">Select an answer...</option>
                                        {condAnswers.map(function(a){return <option key={a} value={a}>{a}</option>;})}
                                      </select>
                                    ) : (
                                      <input id={'cond-a-'+question.id} type="text" value={question.condition_answer||''} onChange={function(e){updateQuestion(question.id,'condition_answer',e.target.value);}} placeholder="Expected answer..." className={inputCls}/>
                                    )}
                                  </div>
                  ...
                  <button type="button" onClick={addQuestion} className="w-full py-3 border-2 border-dashed border-gray-300 text-[#64748B] hover:border-blue-400 hover:text-blue-500 rounded-xl font-semibold text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors flex items-center justify-center gap-2">
          ...
          <div className="flex items-center justify-end gap-3 px-6 py-4 border-t border-slate-200 flex-shrink-0">
            <button type="button" onClick={handleClose} disabled={loading} className="px-6 py-3 bg-transparent border border-slate-300 text-[#475569] font-semibold rounded-lg hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 transition-all disabled:opacity-50 text-sm">Cancel</button>
            <button type="button" onClick={handleSubmit} disabled={loading||editLoading||!formData.title.trim()} className="px-6 py-3 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 transition-all disabled:opacity-50 flex items-center gap-2 text-sm">
              {loading ? (<><svg className="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24" aria-hidden="true"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8H4z"/></svg>{isEditMode?'Saving...':'Creating...'}</>) : (<><Icon path={ICONS.clipboard} className="h-4 w-4"/>{isEditMode?'Save Changes':'Create Survey'}</>)}
            </button>
          </div>
        </div>
      </div>
      ...
      {showTemplatePicker && (
        <TemplatePicker organizationId={organizationId} onLoad={applyTemplate} onClose={function(){setShowTemplatePicker(false);}}/>
      )}
    </>
  );
}
