    ...
    return (
      <div style={{ padding: '18px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {[1, 2, 3].map(function(i) {
          return (
            <div key={i} style={{ height: '64px', borderRadius: '8px', background: '#F1F5F9' }} className="animate-pulse" aria-hidden="true" />
          );
        })}
      </div>
    );
    ...
    <div style={{ padding: '16px' }}>
      ...
      {showFilter && organizations.length > 1 && (
        <div style={{ marginBottom: '12px' }}>
          <label htmlFor="signups-org-filter" style={{ fontSize: '11px', fontWeight: 700, color: TEXT_MUTED, display: 'block', marginBottom: '4px' }}>
            Filter by org
          </label>
          <select
            ...
            style={{ width: '100%', padding: '7px 10px', background: '#FFFFFF', border: '1px solid #CBD5E1', borderRadius: '8px', color: TEXT_PRI, fontSize: '13px', cursor: 'pointer' }}
      ...
      {signups.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '28px 16px' }}>
          <Package size={36} style={{ color: TEXT_MUTED, margin: '0 auto 10px', opacity: 0.5 }} aria-hidden="true" />
          <p style={{ fontSize: '13px', fontWeight: 600, color: TEXT_PRI, marginBottom: '4px' }}>No sign-ups yet</p>
          <p style={{ fontSize: '12px', color: TEXT_MUTED, lineHeight: 1.5 }}>Sign up for volunteer spots or items to see them here.</p>
        </div>
      ) : (
          ...
          {signups.map(function(signup) {
            ...
            return (
              <button
                ...
                style={{ display: 'block', width: '100%', textAlign: 'left', background: ITEM_BG, border: '1px solid ' + BORDER, borderRadius: '8px', padding: '12px', cursor: 'pointer', transition: 'border-color 0.15s' }}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '8px' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '13px', fontWeight: 600, color: TEXT_PRI, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {item?.item_name}
                      </span>
                      {signup.quantity > 1 && (
                    ...
                    <p style={{ fontSize: '12px', color: TEXT_SEC, marginBottom: '6px', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {form?.title}
                    </p>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                      {showFilter && org && (
                        <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: TEXT_MUTED }}>
      ...
      {signups.length > 0 && (
        <p style={{ fontSize: '12px', color: TEXT_MUTED, textAlign: 'center', marginTop: '12px', paddingTop: '12px', borderTop: '1px solid ' + BORDER }}>
